...
def batchnorm_backward(dout, cache):
    ...
    N = x.shape[0]
    dx_hat = gamma * dout  
    dvar = -0.5 * np.sum(dx_hat * x_hat, axis=0) / (var + eps)
    dmean = np.sum(-dx_hat / np.sqrt((var + eps)), axis=0) + dvar * np.sum(-2.0/N * (x - mean), axis=0)
    dx1 = dx_hat / np.sqrt(var + eps)
    dx2 = dvar * (2.0/N) * (x - mean)  # from sample_var
    dx3 = dmean * (1.0 / N * np.ones_like(dout))  # from sample_mean
    dx = dx1 + dx2 + dx3
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    ###########################################################################
    N, D = x.shape
    sample_mean = np.mean(x, axis = 1, keepdims = True)  # dx = np.ones((N, D)) * dsample_mean / D
        #(N,)
...
def layernorm_backward(dout, cache):
    ...
    dgamma = np.sum(dout * x_hat, axis = 0)
    dx_hat = dout * gamma
    dvar = -0.5 * np.sum(dx_hat * x_hat, axis = 1, keepdims = True ) / (var+eps)
    dmean = np.sum(-dx_hat / np.sqrt(var + eps), axis = 1, keepdims = True) + dvar * np.sum(-2.0/D * (x - mean), axis = 1, keepdims = True)
    dx_1 = dx_hat / np.sqrt(var + eps)
    dx_2 = dvar * (2.0/D) * (x - mean)
    dx_3 = dmean * (1.0/D) #* np.ones_like(dout))
    dx = dx_1 + dx_2 + dx_3
    """
    dx_norm = dout * gamma #(N, D)
    dmargin = dx_norm * invert
    dinvert = np.sum(dx_norm * margin, axis = 1, keepdims = True)
    dsqrt = - sqrt**(-2) * dinvert
    dsum = 0.5 * _sum**(-0.5) * dsqrt
    dsample_var = dsum          #(N,)
    dsquare = np.ones((N, D)) * dsample_var / D
    dmargin += 2 * margin * dsquare
    dx = dmargin
    dsample_mean = - np.sum(dmargin, axis = 1, keepdims = True)
    dx += np.ones((N, D)) * dsample_mean / D
    
    dbeta = np.sum(dout, axis = 0)
    dgamma = np.sum(x_norm * dout, axis = 0)
    dx = 1 / N * gamma * invert * (N * dout - x_norm * dgamma - dbeta) 
    """
    #dx = 1 / D * gamma * invert * (D * dout - x_norm * dgamma - dbeta)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dgamma, dbeta
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    cache = (G, x, x_norm, mean, var, gamma, eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    G, x, x_norm, mean, var, gamma, eps = cache
        # dbeta，dgamma
    ...
    dmean2_var = dvar * -2.0 / size_group * np.sum(x_group - mean, axis=(2, 3, 4), keepdims=True)
    dmean = dmean1 + dmean2_var
        # dx_group
    dx_group1 = dx_groupnorm * 1.0 / np.sqrt(var + eps)
    dx_group2_mean = dmean * 1.0 / size_group
    dx_group3_var = dvar * 2.0 / size_group * (x_group - mean)
...
def svm_loss(x, y):
    ...
    num_pos = np.sum(margins > 0, axis=1)  #只将每一行中大于0的值相加
    dx = np.zeros_like(x)
    dx[margins > 0] = 1
