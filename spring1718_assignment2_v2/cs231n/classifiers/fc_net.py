    ...
    def loss(self, X, y=None):
        ...
        for lay in range (self.num_layers - 1):
            if self.normalization=='batchnorm':
                ...
                layer_input, ar_cache[lay] = affine_ln_relu_forward(layer_input, self.params['W%d'%(lay+1)], self.params['b%d'%(lay+1)], self.params['gamma%d'%(lay+1)], self.params['beta%d'%(lay+1)], self.ln_params[lay])
            else:
        ...
        for idx in range(self.num_layers-1):
            ...
            if self.normalization=='batchnorm':
                ...
                dx, dw, db, dgamma, dbeta = affine_ln_relu_backward(dhout, ar_cache[lay])
                grads['gamma%d'%(lay+1)] = dgamma
                grads['beta%d'%(lay+1)] = dbeta
            else:
