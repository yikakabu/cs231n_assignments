...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        #(1)   input: features:(N, D) W_proj:(D, H)    output:(N, H)
        affine_out, affine_cache = affine_forward(features ,W_proj, b_proj)
        
        #(2)   input: captions_in:(N, T) W_embed:(V, W)    output:(N, T, W)
        word_embedding_out, word_embedding_cache = word_embedding_forward(captions_in, W_embed)
        
        #(3)   input: word_embedding_out:(N, T, W) affine_out:(N, H) Wx:(W, H) Wh:(H, H)    output:(N, T, H)
        ...
        #(4)   input: rnn_or_lstm_out:(N, T, H) W_vocab:(H, V)    output:(N, T, V)
        temporal_affine_out, temporal_affine_cache = temporal_affine_forward(rnn_or_lstm_out, W_vocab, b_vocab)
        
        #(5)   input: temporal_affine_out:(N, T, V) captions_out:(N, T) mask:(N, T)
